...
function loadPosts() {
  ...
  return fs.readdirSync(BLOG_DIR).filter(f => f.endsWith(".md")).map(f => {
    const raw = fs.readFileSync(path.join(BLOG_DIR, f), "utf8"), { data, content } = matter(raw), rawHtml = marked.parse(content), { html, toc } = injectHeadingIdsAndToc(rawHtml), slug = slugFromFilename(f);
    return { slug, title: data.title, date: data.date, category: data.category || "Блог", excerpt: data.excerpt, cover: data.cover || BLOG_COVERS[slug] || null, readMinutes: readingTime(content), toc, html: wrapTables(wrapCallouts(html)) };
...
function blogCardMedia(p) {
  ...
  return `<div class="blog-card-media blog-card-media--photo"><img src="${p.cover}${isSvg ? "" : ".jpg"}" alt="${p.title}" loading="lazy"></div>`;
}
