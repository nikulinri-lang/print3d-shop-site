...
function render(distDir){const posts=loadPosts(),blogDir=path.join(distDir,"blog"); // Индекс блога всегда собирается из всех Markdown-статей из content/blog.fs.mkdirSync(blogDir,{recursive:true});fs.writeFileSync(path.join(blogDir,"index.html"),blogIndexPage(posts));for(const post of posts)fs.writeFileSync(path.join(blogDir,`${post.slug}.html`),blogPostPage(post));console.log(`  ✓ blog/index.html + ${posts.length} статей`);return posts;}
