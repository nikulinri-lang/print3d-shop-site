...
function articleCoverHTML(post){if(!post.cover)return"";return`<div class="article-cover"><img src="${post.cover}" alt="${post.title}" loading="eager"></div>`;}
function blogPostPage(post){const body=`<div class="reading-progress" id="readingProgress"></div><section class="page-hero page-hero--article"><div class="container"><span class="kicker">Блог</span><div class="blog-card-date mono">${fmtDate(post.date)} · ${post.readMinutes} мин чтения</div><h1>${post.title}</h1></div></section><section class="section"><div class="container container--article-wide">${articleCoverHTML(post)}<div class="article-layout"><article class="article-content">${post.html}${articleCta()}</article>${tocHTML(post.toc)}</div><a href="/blog" class="btn btn-ghost article-back">← Ко всем статьям</a></div></section>`;const script=`<script>(function(){var bar=document.getElementById('readingProgress');if(!bar)return;function update(){var h=document.documentElement,scrolled=h.scrollTop,height=h.scrollHeight-h.clientHeight,pct=height>0?(scrolled/height)*100:0;bar.style.width=pct+'%';}document.addEventListener('scroll',update,{passive:true});update();})();</script>${post.toc.length>=2?'\n<script defer src="/js/article-toc.js?v=1"></script>':""}`;return renderLayout({title:`${post.title} — Блог PRINTLAB`,description:post.excerpt,canonical:`/blog/${post.slug}`,activeNav:"/blog",bodyContent:body,extraScripts:script});}
function render(distDir){const posts=loadPosts(),blogDir=path.join(distDir,"blog");fs.mkdirSync(blogDir,{recursive:true});fs.writeFileSync(path.join(blogDir,"index.html"),blogIndexPage(posts));for(const post of posts)fs.writeFileSync(path.join(blogDir,`${post.slug}.html`),blogPostPage(post));console.log(`  ✓ blog/index.html + ${posts.length} статей`);return posts;}
