...
function blogCardHTML(p){return`<a href="/blog/${p.slug}" class="blog-card">${blogCardMedia({...p,cover:p.indexCover})}<span class="blog-card-tag">${p.category}</span><div class="blog-card-meta"><span class="blog-card-date mono">${fmtDate(p.date)}</span><span class="blog-card-read-time mono">${p.readMinutes} мин чтения</span></div><h3>${p.title}</h3><p>${p.excerpt}</p></a>`;}
